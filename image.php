				...
				<div class='section-inner'>
					...
					<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
						...
						<nav id="image-navigation" class="navigation image-navigation">
							<div class="nav-links">
								<div class="nav-previous"><?php previous_image_link( false, __( 'Previous Image', 'devbio' ) ); ?></div>
								<div class="nav-next"><?php next_image_link( false, __( 'Next Image', 'devbio' ) ); ?></div>
							</div><!-- .nav-links -->
						...
						<div class="entry-content">
							...
							<div class="entry-attachment">
								<?php
									/**
									 * Filter the default devbio image attachment size.
									 *
									 * @since Developer Bio 1.0
									 *
									 * @param string $image_size Image size. Default 'large'.
									 */
									$image_size = apply_filters( 'devbio_attachment_size', 'large' );

									echo wp_get_attachment_image( get_the_ID(), $image_size );
								?>

								<?php devbio_excerpt( 'entry-caption' ); ?>
						...
						<footer class="entry-footer">
							<?php devbio_entry_meta(); ?>
							<?php
								// Retrieve attachment metadata.
								$metadata = wp_get_attachment_metadata();
								if ( $metadata ) {
									printf( '<span class="full-size-link"><span class="screen-reader-text">%1$s </span><a href="%2$s">%3$s &times; %4$s</a></span>',
										esc_html_x( 'Full size', 'Used before full size attachment link.', 'devbio' ),
